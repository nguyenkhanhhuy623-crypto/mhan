...
local function LoadScript(tier, key)
    ...
    if key and key ~= "" then
        ...
        getgenv().script_key = "QUANTUM_ONYX_" .. tostring(os.time())
    end
...
local function ShowKeyUI()
    ...
    New("TextLabel", {
        ...
        ZIndex = 203,
        Text = "Premium — Tất cả key được chấp nhận.\nNhập bất kỳ key nào để tiếp tục.",
        Parent = NoticeBg
    })
    ...
    local KeyInput = New("TextBox", {
        ...
        Font = Enum.Font.GothamBold,
        PlaceholderText = "Nhập key (tùy chọn)...",
    ...
    local function SubmitKey(keyStr)
        ...
        submitting = true
        SetStatus("Đang xác nhận...", Color3.fromRGB(175, 150, 255))
        task.spawn(function()
            task.wait(0.3)
            submitting = false
            isPremium = true
            resultKey = (keyStr and keyStr ~= "") and keyStr or "QUANTUM_ONYX_" .. tostring(os.time())
            SaveKey(resultKey)
            getgenv().script_key = resultKey
            SetStatus("Xác nhận thành công", Color3.fromRGB(80, 230, 130))
            task.wait(0.5)
            AnimateClose()
        end)
    end
    ...
    MakeBtn("Bản Miễn Phí", RX, BtnW, Color3.fromRGB(35, 14, 70), Color3.fromRGB(170, 130, 255), function()
        isPremium = false
        AnimateClose()
    end)
    ...
    local getKeyBtn = MakeBtn("Lấy Key", RX + BtnW + BtnGap, BtnW, Color3.fromRGB(12, 35, 70), Color3.fromRGB(105, 175, 255), function()
    ...
    MakeBtn("Xác Nhận", RX + (BtnW + BtnGap) * 2, BtnW, Color3.fromRGB(48, 14, 100), Color3.fromRGB(200, 150, 255), function()
